...
from procrastinate import BaseConnector, exceptions, jobs, sql, utils
...
class JobManager:
    ...
    def fetch_job(self, queues: Optional[Iterable[str]]) -> Optional["jobs.Job"]:
        ...
        row = self.connector.execute_query_one(
    ...
    def get_stalled_jobs(
        ...
        ``Iterable[jobs.Job]``
        """
        rows = self.connector.execute_query_all(
    ...
    def wait_for_job(
        self, *, queues: Optional[Iterable[str]] = None, timeout: Optional[float] = None
    ) -> None:
        """
        Waits till notification is received or timeout have passed.
        ...
        timeout : ``Optional[Iterable[float]]``
            Timeout in seconds. If it is ``None``, the call will never time out.
        """
        self.connector.wait_for_notification(
            channels=get_channel_for_queues(queues=queues), timeout=timeout
        )
    ...
    def list_queues(
        ...
        return [
            ...
            for row in self.connector.execute_query_all(
