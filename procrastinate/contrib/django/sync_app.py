...
from procrastinate import exceptions, jobs, schema, utils
from procrastinate.contrib.django import sync_manager

if TYPE_CHECKING:
    from procrastinate.contrib.django import sync_worker
...
class App(blueprints.Blueprint):
    ...
    def __init__(
        ...
        *,
        ...
        self.job_manager = sync_manager.JobManager(connector=self.connector)
    ...
    def _worker(self, **kwargs) -> "sync_worker.Worker":
        from procrastinate.contrib.django import sync_worker

        final_kwargs = {**self.worker_defaults, **kwargs}

        return sync_worker.Worker(app=self, **final_kwargs)
